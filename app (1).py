...
# 记录默认参数
default_params = [{"upscale": 1, "pipeline": ("placeholder", "placeholder"), "pipes": {}}]
...
def inference_sr(image, 
        ...
        name, pipeline = default_params[0]['pipeline']
        if name != "sr-超分" or upscale != default_params[0]['upscale']: 
            del default_params[0]['pipeline']
            ...
            default_params[0]['pipeline'] = ("sr-超分", pipeline)
            default_params[0]['upscale'] = upscale
...
def inference_dn(image, 
        ...
        if name != "dn-降噪": 
            ...
            default_params[0]['pipeline'] = ("dn-降噪", pipeline)
...
def inference_fr(image, 
        ...
        if name != "fr-对齐脸部修复": 
            ...
            default_params[0]['pipeline'] = ("fr-对齐脸部修复", pipeline)
...
def inference_fr_bg(image, 
    ...
    if not default_params[0]['pipes'] or default_params[0]['upscale'] != upscale:
        bsrnet = instantiate_from_config(OmegaConf.load("configs/inference/bsrnet.yaml"))
        sd = load_model_from_url(MODELS["bsrnet"])
        bsrnet.load_state_dict(sd, strict=True)
        bsrnet.eval().cuda()
        ...
        default_params[0]['pipes'] = pipes
        default_params[0]['upscale'] = upscale

    pipes = default_params[0]['pipes']
...
def inference(mode,
    ...
    try:
        if mode == "sr-超分":
            del default_params[0]['pipes']
            default_params[0]['pipes'] = {}
            ...
            del default_params[0]['pipeline']
            default_params[0]["pipeline"] = ("placeholder", "placeholder")
        ...
        del default_params[0]
        default_params[0] = {"upscale": 1, "pipeline": ("placeholder", "placeholder"), "pipes": {}}
        raise e
